...
function exportDrawing(format, action, additionalText) {
  ...
  const downloadFileName = `${colorPalettes[currentPaletteIndex].palette.name}_${date}_${additionalText}`;

  if (format === "png" || format === "jpg") {
    saveCanvas(downloadFileName, format);
  } else {
    console.log(
      `Your input ('${format}') does not match the expected inputs ('png' or 'jpg')`
    );
    return;
  }
  if (action === "noLoop") {
...
document
  .getElementById("noloop-btn")
  .addEventListener("click", () => updateLoopBtn()); // on button click
document
  .getElementById("canvas-div")
  .addEventListener("click", () => updateLoopBtn()); // on canvas click

// Adds event listeners to download the drawing
document
  .getElementById("png-btn")
  .addEventListener("click", () => exportDrawing("png", "", ""));
document
  .getElementById("jpg-btn")
  .addEventListener("click", () => exportDrawing("jpg", "", ""));
...
function loadPalettes() {
  ...
  for (let i = 0; i < colorPalettes.length; i++) {
    ...
    // paletteHtmlElement.style.background = "rgb(0,123,123)";
